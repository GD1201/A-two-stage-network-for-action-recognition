...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self,frame_nums=120,skeleton_nums=18, in_chans=3,patch_size=1, t_patch_size=4,
                 ...
                 mlp_ratio=4., drop_rate=0.,drop_path_rate=0.,attn_drop_rate=0.,norm_layer=nn.LayerNorm,
                 use_mask_tokens=False, norm_pix_loss=True):
        super().__init__()
        ...
        self.patch_Embed = SkeleEmbed(in_chans, dim_feat, frame_nums, skeleton_nums, patch_size, t_patch_size)
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, qk_scale=None,
        ...
        self.decoder_pos_embed_mask=nn.Parameter(torch.zeros(1,self.patch_Embed.num_patches+1,decoder_embed_dim),requires_grad=False)
        ...
        self.decoder_blocks1 = nn.ModuleList([
            CrossAttentionBlock(embed_dim, decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, self_attn=False)
            for i in range(decoder_depth)])
        trunc_normal_(self.pos_embed, std=.02)
        trunc_normal_(self.temp_embed, std=.02)
        trunc_normal_(self.mask_token, std=.02)
        ...
        trunc_normal_(self.decoder_pos_embed_mask, std=.02)
    ...
    def skeleton_ids(self,data_skeleton,dynamic_ratio,static_ratio,L):
        ...
        # assert (C == 12)
        x_1=torch.zeros_like(data_skeleton)  ## 64 30 18 256       建立全为0张量储存相应的运动信息  取绝对值 计算运动高低 由大到小排列
        x_1[:,1:,:,:]=torch.abs(data_skeleton[:, 1:, :, :] -data_skeleton[:, :-1, :, :]) #28
        # x_1=torch.abs(x_1) ## 绝对值  来看大小
        # data_skeleton[:,0:1,:,:]=0
        x_1[:,0,:,:]= x_1[:,1,:,:]##0时刻运动流为0=1
        #x_1
        # x_1=torch.cat([x_1,x_1[:,-1:,:,:]],dim=1) #64 30 18 256
        x_1=x_1.mean(dim=[3]) ## 进行维度转换
        x_1=x_1.reshape(B,T*V)  ### 
        # ids_restore=x_motion.argsort(descending=False).cuda() ## 由大到小排列    64  540 

        # x_motion=x_motion  ### 如何加入噪声...  归一化. (0,1)
        x_1 =x_1/ (torch.max(x_1, dim=-1, keepdim=True).values * 0.85 + 1e-10)
        x_orig_motion_prob = F.softmax(x_1, dim=-1)
        ##64 540
        
        noise = torch.log(x_orig_motion_prob) - torch.log(-torch.log(torch.rand(B, L, device=data_skeleton.device) + 1e-10) + 1e-10) 
        ### 静态、动态比例
        ids_restore=noise.argsort(descending=False).cuda() #### 从大到小排列
        ...
        #19*
        return ids_restore
    def motion_aware_masking(self,x,data_skeleton,mask_ratio,keep_ration):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        ...
        # # sort noise for each sample
        len_keep = int(L * (1 - mask_ratio))
        x_orig_motion = torch.zeros_like(data_skeleton)
        ...
        x_orig_motion = x_orig_motion.reshape(N, L)

        x_orig_motion = x_orig_motion / (torch.max(x_orig_motion, dim=-1, keepdim=True).values * 0.80 + 1e-10)
        x_orig_motion_prob = F.softmax(x_orig_motion, dim=-1)
        noise = torch.log(x_orig_motion_prob) - torch.log(-torch.log(torch.rand(N, L, device=x.device) + 1e-10) + 1e-10)  # gumble
        # noise = torch.rand(N, L, device=x.device)  # noise in [0, 1]
        # noise = self.skeleton_ids(data_skeleton,dynamic_ratio,static_ratio,L) ## 噪声和mae实现方法一样
        # sort noise for each sample
        ...
        return x_masked, mask, ids_restore
    def forward_encoder(self, x, mask_ratio,keep_ratio):
        ...
        if self.use_mask_tokens==True:
            ...
            x=x+self.decoder_pos_embed_mask[:,1:,:]
            # x, mask, ids_restore,ids_keep=self.random_masking(x,mask_ratio)
            x, mask, ids_restore=self.motion_aware_masking1(x, x_copy, mask_ratio - 0.5, 0.5,keep_ratio)
        else :   
            x_copy = self.patchify(x)
            x = self.patch_Embed(x)
            NM, TP, VP, _ = x.shape ## 64 120 )18 256  64 30 18 256
            # x_copy=x_copy.reshape(NM,TP,VP,-1)
            ...
            # if motion_Aware:
            x_copy=x_copy.reshape(NM,TP,VP,-1)
            x, mask, ids_restore=self.motion_aware_masking(x, x_copy,mask_ratio,keep_ratio)
            ###静态+动态
            # x, mask, ids_restore=self.motion_aware_masking1(x, x_copy, mask_ratio - 0.5, 0.5,keep_ratio)
    ...
    def motion_aware_masking1(self,x,data_skeleton,dynamic_ratio,static_ratio,keep_ratio):
        N, L, D = x.shape  # batch, length, dim    64    540  256
        mask_ratio=dynamic_ratio+static_ratio
        len_keep = int(L * (1 - mask_ratio)) ### 可以再从masktoken中 再选取partial进行复原？
        len_masked=int(L*(mask_ratio-keep_ratio))
        # len_keep = int(L * (1 - mask_ratio))
    ...
    def forward_decoder(self, y,mask, ids_restore):
        # embed tokens
        # print(y.shape)  64   112   256
        # NM = x.shape[0]
        # T = self.patch_Embed.t_grid_size
        # V= self.patch_Embed.grid_size
        ...
        # x = x.reshape(NM, T * V, C)
        # # x = x.masked_select(mask.bool().unsqueeze(-1)).reshape(NM, -1, self.mask_token.shape[-1])
    ...
    def forward(self, x, mask_ratio,keep_ratio):
        #img:->skeleton 64 3 300 18
        N, C, T, V, M = x.shape# （32,3,120,18,2）
        x = x.permute(0, 4, 2, 3, 1).contiguous().view(N * M, T, V, C) ## 64 120 18 3
        # x=x[...,0] #32 3  120 18
        # x = x.permute(0, 2, 3, 1).contiguous().view(N, T, V, C)
        # # # x=x[...,0] #(32,3,120,18)  (64,120,25,3)
        x_motion=self.motion(x)  ## 64 
        # latent, mask, ids_restore,coords = self.forward_encoder(x, mask_ratio,kept_mask_ratio) 
        # ##   64  120  18 3
        latent, mask, ids_restore,coords = self.forward_encoder(x, mask_ratio,keep_ratio)
        #64 18 600
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
          frame_nums=120,skeleton_nums=25,patch_size=1, t_patch_size=4, embed_dim=256, depth=8, num_heads=8,
        decoder_embed_dim=256, decoder_depth=3, decoder_num_heads=8,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        skeleton_nums=25,patch_size=6, embed_dim=512, depth=8, num_heads=8,
